...
fit_2 <- kmeans(X, 2) 
DataProportion <- cbind(DataProportion, fit_2$cluster)
DataTotal <- cbind(DataTotal, fit_2$cluster)
fit_2$centers

fit_3 <- kmeans(X, 3) 
DataProportion <- cbind(DataProportion, fit_3$cluster)
DataTotal <- cbind(DataTotal, fit_3$cluster)
fit_3$centers

colnames(DataProportion)[29:30] = c("cluster2", "cluster3")
colnames(DataTotal)[32:33] = c("cluster2", "cluster3")

# DataProportion_fit2_impact1 <- subset(DataProportion, DataProportion$cluster2 ==1 ) 
# DataProportion_fit2_impact2 <- subset(DataProportion, DataProportion$cluster2 ==2 )
# 
# DataProportion_fit3_impact1 <- subset(DataProportion, DataProportion$cluster3 ==1 ) 
# DataProportion_fit3_impact2 <- subset(DataProportion, DataProportion$cluster3 ==2 )
# DataProportion_fit3_impact3 <- subset(DataProportion, DataProportion$cluster3 ==3 )
...
# DataProportion <- DataProportion_fit2_impact1 
# DataProportion <- DataProportion_fit2_impact2 
# DataProportion <- DataProportion_fit3_impact1 
# DataProportion <- DataProportion_fit3_impact2 
# DataProportion <- DataProportion_fit3_impact3 
...
# rm(ConProportion)
# Imp2005 <- 23
# Con2005Proportion <- data.frame (matrix(nrow=length(DataProportion[,1]) , ncol = 23))
# Con2005Proportion[,1] <- DataProportion[,1]
# Con2005Proportion[,2] <- DataProportion[,Imp] # total
# Con2005Proportion[,11] <- DataProportion[,Imp] # total
# 
# for (i in 1: length(DataProportion[,1]) ){
#   for (j in 1: 8){
#     Con2005Proportion[i,2+j] <-  DataProportion[i,1+j] * DataProportion[i,Imp2005]  # Imp 25 = concentration
#   }
#   for (j in 1: 12){
#     Con2005Proportion[i,11+j] <-  DataProportion[i,9+j] * DataProportion[i,Imp2005]  # Imp 25 = concentration
#   }
# }
# 
# colnames(Con2005Proportion)[1:23] <- c("GeoID", "total", "C_white", "C_afri", "C_asian" , "C_his" , 
...
# 
# 
# rm(Con2012Proportion)
# Imp2012 <- 24
# Con2012Proportion <- data.frame (matrix(nrow=length(DataProportion[,1]) , ncol = 23))
# Con2012Proportion[,1] <- DataProportion[,1]
# Con2012Proportion[,2] <- DataProportion[,Imp] # total
# Con2012Proportion[,11] <- DataProportion[,Imp] # total
# 
# for (i in 1: length(DataProportion[,1]) ){
#   for (j in 1: 8){
#     Con2012Proportion[i,2+j] <-  DataProportion[i,1+j] * DataProportion[i,Imp2012]  # Imp 25 = concentration
#   }
#   for (j in 1: 12){
#     Con2012Proportion[i,11+j] <-  DataProportion[i,9+j] * DataProportion[i,Imp2012]  # Imp 25 = concentration
#   }
# }
# 
# colnames(Con2012Proportion)[1:23] <- c("GeoID", "total", "C_white", "C_afri", "C_asian" , "C_his" , 
#                                    "C_nonwhite", "C_nonafri", "C_nonasian" , "C_nonhis" ,"total",
#                                    "C_poor", "C_notpoor",
#                                    "C_incg1", "C_incg2", "C_incg3", "C_incg4",
#                                    "C_nonincg1", "C_nonincg2", "C_nonincg3", "C_nonincg4",
...
# ILATotal <- data.frame()
# for (j in 1: 22) {
#   ILATotal[1,j] <- sum (Con2005Proportion[,j+1])
#   ILATotal[2,j] <- sum (Con2012Proportion[,j+1])
#   ILATotal[3,j] <- sum (ConProportion[,j+1])
#   ILATotal[4,j] <- sum (IncProportion[,j+1])
#   ILATotal[5,j] <- sum (ValProportion[,j+1])
# }
...
write.table(ILATotal,"C:/Users/Kate Hyun/Dropbox/Kate/EJ/paper research/DataAnalysis_2014_AfterTRB/0128R/EJ/Result2_02042015/ILATotal_fit3_region1.txt",
            sep="\t",row.names=FALSE)
write.table(ILARatio,"C:/Users/Kate Hyun/Dropbox/Kate/EJ/paper research/DataAnalysis_2014_AfterTRB/0128R/EJ/Result2_02042015/ILARatio_fit3_region1.txt",
            sep="\t",row.names=FALSE)
write.table(PopTotal,"C:/Users/Kate Hyun/Dropbox/Kate/EJ/paper research/DataAnalysis_2014_AfterTRB/0128R/EJ/Result2_02042015/PopTotal_fit3_region1.txt",
            sep="\t",row.names=FALSE)
